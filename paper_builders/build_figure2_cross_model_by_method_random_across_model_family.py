...
Additionally:
- For recovery plots only, draw a lighter dotted horizontal line for the Random W baseline.
- That line is computed from table_utils.py records:
    random_recovery = Random_W / Gold
- For each family panel (LLaMA / Qwen), the random line is the mean over the three model pairs
  in that family (and over all three methods).
"""
...
def plot_task(metric: str, task: str, vals, out_path: Path, random_lines=None):
    ...
    for ax, (fam, order) in zip(axes, FAMILY_ORDER_ITEMS):
        ...
        if metric == "recovery" and random_lines is not None:
            rv = random_lines.get(fam, np.nan)
            if np.isfinite(rv):
                ax.axhline(rv, linestyle=":", linewidth=1.0, color="black", alpha=0.35)
...
def plot_mean(metric: str, mean_std, out_path: Path, n_tasks_used: int, random_lines=None):
    fig, axes = plt.subplots(1, len(FAMILY_ORDER_ITEMS), figsize=(4.7 * len(FAMILY_ORDER_ITEMS), 3.2), sharey=True)
    axes = axes.ravel()
...
def main():
    ...
    # Load records index for random_W baseline lines
    ...
    for mode in modes:
        ...
        for metric in ("recovery", "gapclosed"):
            ...
            plot_mean(
                ...
                random_lines=random_lines_mean,
            )
