...
def pair_random_recovery_segment(idx, task: str, fam: str, pair: Tuple[float, float]) -> float:
...
def draw_pair_random_segments(ax: plt.Axes, x_positions, order, fam: str, pair_random_segments: Dict):
    if pair_random_segments is None:
        return
    seg_half = 0.39
    for xi, pair in zip(x_positions, order):
        rv = pair_random_segments.get(fam, {}).get(pair, np.nan)
        if np.isfinite(rv):
            ax.hlines(
                rv,
                xi - seg_half,
...
def plot_task(metric: str, task: str, vals, out_path: Path, pair_random_segments=None):
    ...
    for ax, (fam, order) in zip(axes, FAMILY_ORDER_ITEMS):
        ...
        if metric == "recovery":
            draw_pair_random_segments(ax, x, order, fam, pair_random_segments)
...
def plot_mean(metric: str, mean_std, out_path: Path, n_tasks_used: int, pair_random_segments=None):
    fig, axes = plt.subplots(1, len(FAMILY_ORDER_ITEMS), figsize=(4.7 * len(FAMILY_ORDER_ITEMS), 3.2), sharey=True)
    axes = axes.ravel()
    ...
    for mode in modes:
        ...
        for t in tasks:
            ...
            plot_task(
                ...
                pair_random_segments=pair_random_segments,
            )
            plot_task(
                "gapclosed",
                t,
        ...
        for metric in ("recovery", "gapclosed"):
            ...
            pair_random_segments_mean = None
            if metric == "recovery":
                pair_random_segments_mean = {
                    fam: {
            ...
            plot_mean(
                metric,
                mean_std,
                out_dir / f"figure2_{metric}_mean_over_tasks_by_method",
                n_tasks_used=len(vals_list),
                pair_random_segments=pair_random_segments_mean,
            )
