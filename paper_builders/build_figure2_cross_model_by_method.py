...
def plot_task(metric: str, task: str, vals, out_path: Path, random_ref=None, random_line_mode="none"):
    ...
    fig.legend(handles=method_legend_handles(), loc="upper center", ncol=3, frameon=True, bbox_to_anchor=(0.5, 0.98))
    fig.tight_layout()
...
def plot_mean(metric: str, mean_std, no_title: bool, out_path: Path, n_tasks_used: int, random_ref=None, random_line_mode="none"):
    fig, axes = plt.subplots(1, len(FAMILY_ORDER_ITEMS), figsize=(4.7 * len(FAMILY_ORDER_ITEMS), 3.2), sharey=True)
    axes = axes.ravel()
