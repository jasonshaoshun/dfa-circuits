    ...
    for model_name in args.models:
        ...
        for task in args.tasks:
            if f"{task.replace('_', '-')}_{model_name}" not in COL_MAPPING:
                print(f"Skipping {task} with {model_name} as it's not in the COL_MAPPING.")
                continue
            graph = Graph.from_model(model, neuron_level=neuron_level, node_scores=node_scores)
            print(f"\n\nCreated graph with {len(graph.nodes)} nodes and {len(graph.edges)} edges for {task} with {model_name}.\n\n")
            ...
            if args.level == 'edge':
                ...
                # if model_name in ("llama3-8b") and task in ("arc_easy", "arc_challenge"):
                #     print(f"Using 3 IG steps for {model_name} on {task} due to computational constraints.")
                #     attribute_node(model, graph, dataloader, attribution_metric, args.method, 
                #                     args.ablation, neuron=args.level == 'neuron', ig_steps=3,
