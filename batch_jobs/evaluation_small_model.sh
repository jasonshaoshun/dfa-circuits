...
for pair in "${MODEL_PAIRS[@]}"; do
  ...
  for mla in "${METHODS_LEVEL_ABLATIONS[@]}"; do
    ...
    # Include the two smallest models for gold evaluation, since they are only
    # source models in MODEL_PAIRS and would otherwise be skipped.
    if [[ "${source_model}" == "llama3-1b" && "${target_model}" == "llama3-3b" ]]; then
      extra_gold_model="llama3-1b"
    elif [[ "${source_model}" == "qwen2.5-0.5b" && "${target_model}" == "qwen2.5-1.5b" ]]; then
      extra_gold_model="qwen2.5-0.5b"
    else
      extra_gold_model=""
    fi

    if [[ -n "${extra_gold_model}" ]]; then
      extra_safe_target="$(safe_tag "${extra_gold_model}")"
      extra_gold_marker_file="${gold_result_dir}/${marker_task}_${extra_gold_model}_${split}_abs-${absolute}.pkl"
      if [[ -f "${extra_gold_marker_file}" ]]; then
        echo "SKIP evaluation gold ${extra_gold_model} | ${method} | ${ablation} | ${level} (found ${extra_gold_marker_file})"
      else
        echo "file not found: ${extra_gold_marker_file}"
        submit_eval \
          "eval_gold_${extra_safe_target}_${safe_method}_${safe_ablation}_${safe_level}" \
          --models "${extra_gold_model}" \
          --tasks "${TASKS[@]}" \
          --method "${method}" \
          --ablation "${ablation}" \
          --batch-size "${batch_size}" \
