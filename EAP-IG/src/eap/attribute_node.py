...
def get_scores_ig_activations(model: HookedTransformer, graph: Graph, dataloader: DataLoader, metric: Callable[[Tensor], Tensor], 
    ...
    for clean, corrupted, label in dataloader:
        ...
        with model.hooks(fwd_hooks=fwd_hooks_clean):
            clean_logits = model(clean_tokens, attention_mask=attention_mask)
            activation_difference.add_(activations_corrupted.detach())
            activation_difference.sub_(activations_clean.detach())
        ...
        for node in nodeslist:
            for step in range(1, steps+1):
                ...
                with model.hooks(fwd_hooks=fwd_hooks, bwd_hooks=bwd_hooks):
                    ...
                    del metric_value, logits
        
        del activations_clean, activations_corrupted, activation_difference, clean_logits
