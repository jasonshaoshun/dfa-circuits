...
def evaluate_area_under_curve(model: HookedTransformer, graph: Graph, dataloader, metrics, quiet:bool=False, 
    ...
    print(
        "Raw scores: "
        f"baseline_score={baseline_score:.10g}, "
        f"corrupted_score={corrupted_score:.10g}, "
        f"denominator={baseline_score - corrupted_score:.10g}",
        flush=True,
    )
    ...
    for pct in percentages:
        ...
        print(
            "Raw scores: "
            f"pct={pct:.3g}, "
            f"n_{level}s={curr_num_items}, "
            f"weighted_edge_count={weighted_edge_count}, "
            f"ablated_score={ablated_score:.10g}, "
            f"faithfulness={faithfulness:.10g}",
            flush=True,
        )
    ...
    return weighted_edge_counts, area_under, area_from_1, average, faithfulnesses, raw_scores
